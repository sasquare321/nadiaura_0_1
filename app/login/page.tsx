...
export default function LoginPage() {
  ...
  const handleSendOTP = async () => {
    ...
    try {
      ...
      localStorage.setItem('nadiaura-pending-user', JSON.stringify(data.user));
      router.push('/verify');
    } catch {
    ...
    <div className="screen-scroll relative overflow-hidden flex flex-col min-h-full p-6 pb-8"
      ...
      <div className="animate-fade-in flex flex-col items-center mt-7 mb-9">
        <div className="relative mb-6">
          <div className="w-[88px] h-[88px] rounded-[28px] flex items-center justify-center"
            style={{
              background: 'linear-gradient(135deg, var(--accent-dark), var(--accent-subtle))',
              border: '1px solid var(--border)',
              boxShadow: '0 8px 32px var(--accent-glow)',
            }}>
            <Image src="/Icons/nadi.png" alt="Nadi" width={56} height={56} priority
              className="select-none" style={{ objectFit: 'contain' }} />
          </div>
      ...
      <div className="animate-fade-in delay-2 mb-4">
        ...
        <div className="relative mb-[14px]">
          <input
            ...
            style={{
              ...
              border: `1.5px solid ${error ? 'var(--danger)' : focused ? 'var(--accent)' : 'var(--border-subtle)'}`,
              color: 'var(--text-primary)',
              boxShadow: focused ? '0 0 0 3px var(--accent-subtle)' : 'none',
            }}
          />
          <div className="absolute right-3.5 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full flex items-center justify-center"
        ...
        {error && (
          <p className="text-[13px] font-semibold mb-3 text-center" style={{ color: 'var(--danger)' }}>
            ⚠️ {error}
          </p>
        )}

        <button onClick={handleSendOTP} disabled={!input.trim() || loading}
          ...
          style={{
            background: input && !loading ? 'linear-gradient(135deg, var(--accent-dim), var(--accent))' : 'var(--bg-card-3)',
            color: input && !loading ? '#fff' : 'var(--text-muted)',
            boxShadow: input && !loading ? '0 4px 20px var(--accent-glow)' : 'none',
            opacity: loading ? 0.7 : 1,
          }}>
          {loading ? '⏳ Looking up your account…' : <>Continue with OTP <span className="opacity-80">→</span></>}
        </button>
      </div>
      ...
      <div className="text-center mb-3">
        ...
        <Link href="/signup" className="text-[var(--accent)] font-bold text-sm no-underline">Create Account</Link>
      </div>
